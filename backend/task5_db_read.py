# TASK 5: Web RPC Database Queries
from flask import jsonify
from task4_db_router import get_connection, read_from_txt

def setup_read_routes(app):
    """We wrap your routes in a function so task1_web_server can import them directly."""
    ...
    def get_full_quiz(db_type, quiz_id):
        ...
        try:
            with conn.cursor(dictionary=True) as cursor:
                ...
                for q in questions:
                    cursor.execute("SELECT Option_ID, Option_text, Is_correct FROM Options WHERE Question_ID = %s", (q['Question_ID'],))
